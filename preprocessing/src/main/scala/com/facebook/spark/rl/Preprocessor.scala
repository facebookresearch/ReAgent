...
object Preprocessor {
  ...
  def main(args: Array[String]) {
    ...
    val mapStringDoubleToLongDouble = udf((r:Map[String,Double]) => r.map({ case (key, value) => (key.toLong, value) }))

    inputDf = inputDf.withColumn("state_features", mapStringDoubleToLongDouble(inputDf.col("state_features")))
    if (!timelineConfig.actionDiscrete) {
      inputDf = inputDf.withColumn("action", mapStringDoubleToLongDouble(inputDf.col("action")))

      val mapArrayStringDoubleToArrayLongDouble = udf((r:Array[Map[String,Double]]) => r.map((m) => m.map({ case (key, value) => (key.toLong, value) })))
      inputDf = inputDf.withColumn("possible_actions", mapArrayStringDoubleToArrayLongDouble(inputDf.col("possible_actions")))
    }
