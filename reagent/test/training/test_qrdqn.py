...
class TestQRDQN(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertTrue((trainer.quantiles == quantiles).all())
        self.assertTrue((trainer.reward_boosts == torch.zeros(2)).all())
        ...
        self.assertTrue(
            (reward_boost_trainer.reward_boosts == torch.tensor([1, 2])).all()
        )
    ...
    def test_train_step_gen(self):
        ...
        self.assertEqual(
            type(losses[0].grad_fn),
            type(torch.tensor([1.0, 2.0, 3.0], requires_grad=True).mean().grad_fn),
        )
        self.assertEqual(
            type(losses[1].grad_fn),
            type(
                torch.nn.functional.mse_loss(
                    torch.tensor([1.0, 2.0, 3.0], requires_grad=True), torch.zeros(3)
                ).grad_fn
            ),
        )
        self.assertEqual(
            type(losses[2].grad_fn),
            type(
                torch.nn.functional.mse_loss(
                    torch.tensor([1.0, 2.0, 3.0], requires_grad=True), torch.zeros(3)
                ).grad_fn
    ...
    def test_configure_optimizers(self):
        ...
        p1 = optimizers[0]["optimizer"].param_groups[0]["params"][0]
        p2 = list(trainer.q_network.parameters())[0]
        self.assertTrue(torch.all(p1 == p2))
        p3 = optimizers[1]["optimizer"].param_groups[0]["params"][0]
        p4 = list(trainer.reward_network.parameters())[0]
        self.assertTrue(torch.all(p3 == p4))
        p5 = optimizers[2]["optimizer"].param_groups[0]["params"][0]
        p6 = list(trainer.q_network_cpe.parameters())[0]
        self.assertTrue(torch.all(p5 == p6))
        p7 = optimizers[3]["optimizer"].param_groups[0]["params"][0]
        p8 = list(trainer.q_network.parameters())[0]
        self.assertTrue(torch.all(p7 == p8))
