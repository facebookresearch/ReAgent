...
def make_default_score_fn(fn_i: int) -> SCORE_FUNCTION_T:
    ...
    def fn(user: np.ndarray, doc: np.ndarray) -> float:
        assert np.all(np.abs(user) <= 1.0), f"{user} has weights w. abs value > 1"
        assert (
            user.shape == doc.shape and len(user.shape) == 1
        ), f"{user.shape}, {doc.shape}"
        assert user.shape[0] > fn_i, f"{user.shape} is less than {fn_i}"
        return user[fn_i]
        # return doc[fn_i]
        # user = user ** (fn_i + 1)
        # doc = doc ** (fn_i + 1)
...
def get_ground_truth_weights(num_weights):
    return np.linspace(VM_WEIGHT_LOW, VM_WEIGHT_HIGH, num_weights + 1)[1:]
...
class OraclePVM(RecSim):
    ...
    def step(self, action):
        ...
        return self.prev_obs, reward, t, i
