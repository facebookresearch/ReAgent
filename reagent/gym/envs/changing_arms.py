...
The reward for picking an action is the "mu_changes" corresponding to that arm.
...
INITIAL_MUS = torch.tensor([100.0] * 5)
MU_LOW = 0.0
MU_HIGH = 1000.0

MU_CHANGES = torch.tensor([-10.0] * 5)
LEGAL_PROBS = torch.tensor([0.9, 0.8, 0.7, 0.6, 0.5])
...
class ChangingArms(gym.Env):
    ...
    def get_mu_changes(self):
        return MU_CHANGES

    def get_legal_indices_mask(self):
    ...
    def state(self):
        ...
        """
        self.mu_changes = self.get_mu_changes()
        legal_indices_mask = self.get_legal_indices_mask()
    ...
    def state_feature_config_provider(self) -> ModelFeatureConfigProvider__Union:
        ...
        raw = RawModelFeatureConfigProvider(
            ...
            id_mapping_config={
                ...
                "mu_changes": rlt.IdMapping(ids=[0, 1, 2, 3, 4]),
            },
        )
