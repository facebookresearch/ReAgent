...
class SACTrainer(RLTrainerMixin, ReAgentLightningModule):
    ...
    def train_step_gen(self, training_batch: rlt.PolicyNetworkInput, batch_idx: int):
        ...
        actor_log_prob = actor_output.log_prob.clamp(-20.0, 20.0)
        ...
        if self.alpha_optimizer is not None:
            alpha_loss = -(
                (
                    self.log_alpha
                    * (
                        actor_output.log_prob.clamp(-20.0, 20.0) + self.target_entropy
                    ).detach()
                ).mean()
            )
            yield alpha_loss
        ...
        if self.value_network is not None:
            ...
            if self.logged_action_uniform_prior:
                ...
                log_prob_a = actor_output.log_prob.clamp(-20.0, 20.0)
