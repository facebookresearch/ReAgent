...
class GenericConvNetwork(nn.Module):
    def __init__(self, cnn_parameters, layers, activations) -> None:
        super(GenericConvNetwork, self).__init__()
        ...
        conv_out = self.conv_forward(Variable(torch.ones(1, *input_size)))
        self.fc_input_dim = int(np.prod(conv_out.size()[1:]))
        layers[0] = self.fc_input_dim
        self.feed_forward = GenericFeedForwardNetwork(layers, activations)
