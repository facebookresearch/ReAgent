...
def epsilon_greedy_onehot(
    action_discrete, n_action_outputs, epsilon=DEFAULT_EPSILON
):
    if np.random.rand() < epsilon:
...
def add_parameter_update_ops(
    model, optimizer_input="SGD", base_learning_rate=0.01
):
...
def sample_memories(replay_memory, batch_size):
    ...
    return cols[0], cols[1], cols[2].reshape(-1, 1), cols[3].reshape(
        -1, 1
    ), cols[4], cols[5]
