...
def _build_policy(env, predictor, epsilon):
    ...
    def policy(state) -> Tuple[str, float]:
        if np.random.random() < epsilon:
            return (
                np.random.choice(env.ACTIONS),  # type: ignore
                epsilon / (len(env.ACTIONS) - 1),
            )
        else:
            return policy_vector[state], 1.0  # type: ignore
...
def _collect_samples(env, policy, num_steps, initial_state):
    ...
    next_action, next_propensity = policy(next_state)
...
class TestLimitedActionGridworld(unittest.TestCase):
    def setUp(self):
        ...
        self._rl_parameters_all_cheat_maxq = RLParameters(
            gamma=DISCOUNT, target_update_rate=1.0, reward_burnin=10, maxq_learning=True
        )
    ...
    def test_pure_q_learning_all_cheat(self):
        ...
        trainer = DiscreteActionTrainer(q_learning_parameters, self._env.normalization)

        predictor = trainer.predictor()

        policy = _build_policy(self._env, predictor, 1)
        initial_state = self._env.reset()
        iteration_result = _collect_samples(self._env, policy, 20000, initial_state)
        num_iterations = 50
        ...
        initial_state = self._env.reset()
        iteration_result = _collect_samples(self._env, policy, 1000, initial_state)
    ...
    def test_q_learning_limited(self):
        ...
        trainer = LimitedActionDiscreteActionTrainer(
            q_learning_parameters, self._env.normalization
        )
        ...
        for iteration in range(num_iterations):
            ...
            print(
                "iter: {} ({}), ratio: {}, steps to solve: {}, quantile: {}".format(
                    iteration,
                    num_steps,
        ...
        optimality_ratio = np.sum(np.array(evaluation_results.lengths) <= 14) / len(
        ...
        print("ACCURACY", evaluation_results.cheat_ratio, target_cheat_percentage)
        self.assertTrue(
            accuracy < 0.4
