...
class GridworldEnumEvaluator(object):
    def __init__(
        self,
        env,
        assume_optimal_policy: bool,
    ...
    def evaluate(self, predictor):
        examples_by_feature = {'f': []}
        for test_state in self._test_states:
            examples_by_feature['f'].append(float(list(test_state.keys())[0]))
        examples_by_feature['f'] = np.array(examples_by_feature['f'], dtype=np.float32)
        prediction = predictor.predict(examples_by_feature)
        error_sum = 0.0
        for x in range(len(self._test_states)):
            ground_truth = self._test_values[x]
...
class GridworldContinuousEnumEvaluator(GridworldEvaluator):
    def evaluate(self, predictor):
        examples_by_feature = {'f': []}
        for test_state in self._test_states:
            examples_by_feature['f'].append(float(list(test_state.keys())[0]))
